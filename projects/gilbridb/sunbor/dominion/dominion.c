...
void adventurerCard(struct gameState *state){
	...
	//variables from cardEffect function
	int drawntreasure = 0;
	int currentPlayer = whoseTurn(state);
	int cardDrawn;
...
void salvagerCard(int choice1, struct gameState *state, int handPos){
	//declare variables
	int currentPlayer = whoseTurn(state);
	...
	//gain coins equal to trashed card value
...
void sea_hagCard(struct gameState *state){
	//declare variables
	int currentPlayer = whoseTurn(state);
	int i;
...
/*      //+1 buy
      ...
      if (choice1)
	{
      ...
      discardCard(handPos, currentPlayer, state, 0);
*/
      salvagerCard(choice1, state, handPos);
      return 0;
