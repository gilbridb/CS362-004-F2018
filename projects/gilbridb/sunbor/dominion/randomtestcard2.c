...
void embargoRandomTest()
{
	...
	int card_in_test = embargo;
	int card_pos = 0;
	int choice1 = 0;
	k[0] = card_in_test; // Make sure embargo is a part of the set
	for(i = 1; i < 10; i++)
	{
	...
	*/
	for(i=0; i<17; i++)
	{
		...
		if(post->supplyCount[cards[i]] == 0)
		{
			post->supplyCount[cards[i]] = -1; //turn empties to -1 as it is in code
		}
	}
	...
	//...all possible combos
	j = 9;//amount of possible embargo tokens in a game, save 1 for the card played
	for(i = 0; i < 17; i++)
	{
		if(j > 0)
		{
	...
	//Randomize choice1
	num = rand() % 17;
	choice1 = cards[num];//give random card stack that can exist in the game
	...
	int holder = 0;
	int holder_expected = 0;
	holder = cardEffect(card_in_test, choice1,0,0,&gamepost,card_pos, bonus);


	//Alter pre to be what is expected of post

	//Supply count is incorrect, return -1
	if(pre->supplyCount[choice1] == -1)
	{
		pre->coins += 2;
		holder_expected = -1;
	}
	//Supply count correct, return 0
	else
	{
		holder_expected = 0;
		pre->coins += 2;
		...
		//card not added to the played pile as it is trashed
...
//SMITHY STUFF OLD END HERE ---------------------------------------------------------------
*/
	//assert(memcmp(&gamepre, post, sizeof(struct gameState)) == 0);
	if((memcmp(&gamepre, post, sizeof(struct gameState)) != 0) || (holder_expected != holder))
	{
		printf("Test failed on #%i\nNumPlayers: %i, Chosen_Player: %i, hand_pos: %i, choice1: %i\n", times_run, pre->numPlayers, chosen_player, card_pos, choice1);
		printf("Embargo game state for player: Deck Count: %i, Discard Count: %i, Hand Count: %i,\n PlayedCardCount: %i, Coin: %i, Action: %i, Buy: %i,\nChoice1 Embargo Count: %i, Return value: %i\n", post->deckCount[chosen_player], post->discardCount[chosen_player], post->handCount[chosen_player], post->playedCardCount, post->coins, post->numActions, post->numBuys, post->embargoTokens[choice1], holder);
		printf("What game state should be for player: Deck Count: %i, Discard Count: %i, Hand Count: %i,\n PlayedCardCount: %i, Coin: %i, Action: %i, Buy: %i,\nChoice1 Embargo Count: %i, Return value: %i\n", pre->deckCount[chosen_player], pre->discardCount[chosen_player], pre->handCount[chosen_player], pre->playedCardCount, pre->coins, pre->numActions, pre->numBuys, pre->embargoTokens[choice1], holder_expected);
		success--;
		printf("\n\n");
	}
...
{
	printf("Beginning random Test 2 for Embargo().\n");
	...
	printf("Finished random Test 2 for Embargo().\n\n\n\n\n");
	return 0;
}
